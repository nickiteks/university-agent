...
10. Действия, которые могут изменить состояние, выполняются только с учётом прав и подтверждения пользователя.
11. В конце пишется audit log и возвращается итоговый ответ с источниками.
...
vllm-client/
  vllm_usage.py    - клиент Agent/MCP tool -> Envoy -> LiteLLM -> vLLM
...
Минимальный OpenAI-compatible клиент. Агент и MCP tools не ходят напрямую в vLLM, а отправляют запросы в gateway:
